    ...
    def __call__(self, *data, save_path=None):
        ...
        for i in range(1,len(data_list)):
            ...
            if image_dimension != new_dimension:
                raise ValueError("The dimension of datas are not the same. \n"
                    "Please make the dimension the same")
            if image_dimension == 2:
                if (image_array.shape[0:2] != new_array.shape[0:2]):
                    raise ValueError(
                        "The height or width of Data1 and Data%d are not the same. \n" 
                        "Please make the height and width the same." %(i+1))
            ...
            if image_dimension == 3:
                ...
                if (image_array.shape[0:3] != new_array.shape[0:3]):
                    raise ValueError(
                        "The height, width and volumn of Data1 and Data%d are not the same. \n"
                        "Please make the height, width and volumn the same." %(i+1))
